...
object Compressor {
    ...
    private const val INVALID_RESOLUTION =
            "The maximum height and width is already greater than video's height and width " +
                    "try to set another maximum height and width values"
...
//
        ...
        if (durationData.isNullOrEmpty() || bitrateData.isNullOrEmpty()) { //rotationData.isNullOrEmpty()
            // Exit execution
            return Result(
                    success = false,
...
//            return Result(success = false, failureMessage = INVALID_BITRATE)
        ...
        // Check for minimum height and width
        if (height <= maxHeight && width <= maxWidth) {
            return Result(success = false, failureMessage = INVALID_RESOLUTION)
        }
...
//        var newHeight = maxHeight
        ...
        val newBitrate: Int =
                if (configuration.videoBitrate == null) getBitrate(bitrate, newWidth, newHeight, height, width)
    ...
    private fun start(
        ...
        if (newWidth != 0 && newHeight != 0) {
            ...
            try {
                ...
                // Setup mp4 movie
                val movie = setUpMP4Movie(0, cacheFile)
